// WorkflowDesigner.jsx - Updated to handle node selection and settings
...
export default function WorkflowDesigner({ 
  ...
  const [selectedNode, setSelectedNode] = useState<string | null>(externalSelectedNodeId || null);
  ...
  // Sync external and internal selected node state
  useEffect(() => {
    if (externalSelectedNodeId !== undefined && externalSelectedNodeId !== selectedNode) {
      setSelectedNode(externalSelectedNodeId);
    }
  ...
  const handleNodeMouseDown = useCallback((node: WorkflowNode, e: React.MouseEvent) => {
    ...
    if (isConnecting) {
      ...
      setDraggedNode(node);
      const newSelectedNode = node.id === selectedNode ? null : node.id;
      setSelectedNode(newSelectedNode);
      
      // Notify parent component about node selection
      if (onNodeSelected) {
        onNodeSelected(newSelectedNode);
      }
  ...
  }, [isConnecting, connectingSource, isEditing, edges, onEdgesChange, selectedNode, onNodeSelected]);
  ...
  const handleBackgroundClick = useCallback((e: React.MouseEvent) => {
    ...
    if (!e.defaultPrevented) {
      ...
      // Notify parent component about node deselection
      if (onNodeSelected) {
        onNodeSelected(null);
      }
    }
  }, [isConnecting, onNodeSelected]);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (nodeData) {
      // Use the full node data if available
      try {
  ...
  }, [nodes, edges, selectedNode, onNodesChange, onEdgesChange, onNodeSelected]);
    ...
    <div className="h-full">
      ...
      >
        ...
        {nodes.map(node => {
          ...
          const nodeColor = node.color || (node.type === 'start' ? '#22C55E' : 
                                           node.type === 'fork' ? '#3B82F6' : 
                                           node.type === 'end' ? '#EF4444' : '#6B7280');
...
// WorkflowToolbox.jsx - Updated for task management

import React, { useState, useEffect, useCallback, memo } from 'react';
import { Plus, X, Search, Code } from 'lucide-react';

interface Props {
  onDragStart: (type: string, name: string, data?: any) => (e: React.DragEvent) => void;
  onAddTasksFromJson: (data: any[]) => void;
  availableTasks?: any[];
}
...
function WorkflowToolbox({ onDragStart, onAddTasksFromJson, availableTasks = [] }: Props) {
  ...
  useEffect(() => {
    if (availableTasks.length > 0) {
      ...
      setNodeTypes(initialNodeTypes);
    }
  }, [availableTasks]);

  // Handle adding task from JSON
  const handleAddTasksFromJson = useCallback(() => {
    try {
      ...
      // Process tasks and add them to the task list
      onAddTasksFromJson(data);
          ...
          {filteredNodeTypes.map((node) => (
            <div
              ...
              onDragStart={onDragStart(
                node.type, 
                node.name, 
                node.data ? JSON.stringify(node.data) : undefined
              )}
            >
              <div
  ...
  "timeoutMilliseconds": 1000
            ...
            <div className="text-xs text-gray-500 mt-2 mb-4">
              <p>You can add a single task or an array of tasks. Tasks will be added to the available tasks list for reuse.</p>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                onClick={handleAddTasksFromJson}
...
// CreateWorkflow.jsx - Updated to handle node settings
...
// Shared task store (singleton to ensure task persistence)
class TaskStore {
  private static instance: TaskStore;
  private tasks: WorkflowNode[] = [];
  ...
  public getTasks(): WorkflowNode[] {
    return [...this.tasks];
  }

  // CreateWorkflow.jsx - Continued

  public addTask(task: WorkflowNode): void {
    // Check if task with same text already exists
    const exists = this.tasks.some(t => t.text === task.text);
    if (!exists) {
  ...
  public addTasks(tasks: WorkflowNode[]): void {
    let changed = false;
    tasks.forEach(task => {
      // Check if task with same text already exists
      const exists = this.tasks.some(t => t.text === task.text);
...
// Create the global task store instance
const globalTaskStore = TaskStore.getInstance();
...
export default function CreateWorkflow() {
  ...
  useEffect(() => {
    setAvailableTasks(globalTaskStore.getTasks());
  ...
  const handleNodeSelection = useCallback((nodeId: string | null) => {
    ...
    // Auto-switch to settings tab when a node is selected
    if (nodeId) {
  ...
  const parseWorkflow = (workflow: any[]) => {
    ...
    const processNode = (
      ...
      // Also add to task store
      globalTaskStore.addTask({
        ...newNode,
        position: { x: 0, y: 0 } // Reset position for task store
      });
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (json.nodes && json.edges) {
            ...
            // Also add nodes to task store
            json.nodes.forEach((node: WorkflowNode) => {
              const taskNode = {
                ...node,
                position: { x: 0, y: 0 } // Reset position for task store
              };
              globalTaskStore.addTask(taskNode);
            });
  ...
  // Handle adding tasks from JSON (without adding to canvas)
  const handleAddTasksFromJson = useCallback((tasks: any[]) => {
    const processedTasks = tasks.map(task => {
      return {
        id: crypto.randomUUID(),
        type: "rectangle",
        text: task.node || task.text || "New Task",
        url: task.url || "",
        ...
        position: { x: 0, y: 0 }, // Not important for task store
        size: { width: 120, height: 60 },
        borderRadius: 10,
      };
    });
    
    // Add to global task store
    globalTaskStore.addTasks(processedTasks);
  }, []);
  ...
  // Get the currently selected node for the settings panel
      ...
      <div className="p-6 overflow-y-auto space-y-5">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              updateNode(selectedNode.id, { url: e.target.value });
            }}
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          ...
          <textarea
            ...
            onChange={(e) => {
              try {
                ...
                updateNode(selectedNode.id, { headers });
              } catch (error) {
              ...
              <div key={index} className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateNode(selectedNode.id, { inputParameters: newParams });
                  }}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updateNode(selectedNode.id, { retryCount });
              }}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              min="1"
            />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updateNode(selectedNode.id, { retryDelaySeconds });
              }}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              min="1"
            />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updateNode(selectedNode.id, { timeoutMilliseconds });
              }}
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              min="100"
              step="100"
  ...
  return (
      <div className="h-screen flex flex-col bg-gray-50">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left Panel - Workflow Toolbox and Designer */}
          <div className="flex flex-1 overflow-hidden">
            {/* Toolbox */}
            <div className="w-64 border-r border-gray-200 bg-white">
              <WorkflowToolbox 
                availableTasks={availableTasks}
                onAddTasksFromJson={handleAddTasksFromJson}
                onDragStart={(type, name, data) => (e: React.DragEvent) => {
                  e.dataTransfer.setData('nodeType', type);
                  e.dataTransfer.setData('nodeName', name || '');
                  if (data) {
                    e.dataTransfer.setData('nodeData', data);
                  }
                }}
              />
            </div>
          ...
          <div className="w-1/3 p-4 overflow-auto bg-gray-50 border-l border-gray-200">
...
// EditWorkflow.jsx - Updated to use shared task store and node settings

import React, { useState, useEffect, useRef, useCallback } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, Download, Save, ChevronDown, X } from 'lucide-react';
...
// Import TaskStore from CreateWorkflow 
// This is where we'd normally use a proper state management solution like Redux
// But for simplicity, we're reusing the same class
// In a real app, move this to a shared location
const globalTaskStore = (window as any).globalTaskStore || (() => {
  class TaskStore {
    private static instance: TaskStore;
    private tasks: WorkflowNode[] = [];
    private listeners: (() => void)[] = [];
  ...
  (window as any).globalTaskStore = store;
  return store;
})();
...
export default function EditWorkflow() {
  ...
  // EditWorkflow.jsx - Continued 

  // Generate workflow key (continued)
  const generateKey = useCallback(() => {
    const values = { ...keyFields };
    const sortedPlacements = [...values.placement].sort();
    const placementKey = sortedPlacements.length > 0 ? sortedPlacements.join('-') : '';
  ...
  }, [workflowName, description, generateKey, createNodeHierarchy, workflowStatus]);
  ...
  }, [availableMarkets, availableLanguages, availableClients, availableChannels, availablePages, availableDomains, filteredPlacements, keyFields, optionsLoading, placementSearch, isPlacementOpen, togglePlacement, handleKeyFieldChange]);
